...
class DailyReportService {
    ...
    static async generateAndSendReport() {
        try {
            logger_1.default.info('Generating daily digest report...');
            ...
            logger_1.default.info('Daily digest report sent successfully');
        }
        catch (error) {
            logger_1.default.error('Failed to send daily digest:', error);
        }
    }
    /**
     * Compile report data from database
     */
    static async compileReport() {
        const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000);
        try {
            // 1. System uptime
            const [uptimeResult] = await database_1.sequelize.query(`
        SELECT
      ...
      `, { replacements: [yesterday], type: 'SELECT' });
            ...
            const [remediationActions] = await database_1.sequelize.query(`
      ...
      `, { replacements: [yesterday], type: 'SELECT' });
            // 3. Alerts summary
            const [alertsSummary] = await database_1.sequelize.query(`
      ...
      `, { replacements: [yesterday], type: 'SELECT' });
            ...
            const [resourceAvg] = await database_1.sequelize.query(`
        SELECT
        ...
        WHERE timestamp >= ?
      `, { replacements: [yesterday], type: 'SELECT' });
            const resources = resourceAvg[0] || {};
            // 5. Conversion activity
            const [conversionStats] = await database_1.sequelize.query(`
        SELECT
      ...
      `, { replacements: [yesterday], type: 'SELECT' });
            return {
                ...
                conversions: conversionStats[0] || { total_conversions: 0, successful: 0, failed: 0 },
                date: new Date().toLocaleDateString()
            };
        }
        catch (error) {
            logger_1.default.error('Error compiling report data:', error);
            throw error;
        }
    }
    /**
     ...
     */
    static formatReportHTML(report) {
    ...
    <div class="section">
      ...
      ${report.alerts.length > 0 ? `
        <table>
          ...
          ${report.alerts.map((a) => `
            <tr>
              <td style="text-transform: uppercase; font-weight: bold; color: ${a.severity === 'urgent' ? '#ef4444' :
            a.severity === 'critical' ? '#f59e0b' :
                a.severity === 'warning' ? '#3b82f6' : '#666'}">${a.severity}</td>
              <td>${a.count}</td>
            </tr>
          `).join('')}
        </table>
...
exports.default = DailyReportService;
