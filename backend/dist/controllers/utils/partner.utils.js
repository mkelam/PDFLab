...
async function generateSlug(name) {
    ...
    let counter = 1;
    while (true) {
        const existing = await Partner_1.Partner.findOne({ where: { slug } });
        if (!existing) {
            return slug;
        }
...
async function generateReferralCode(name) {
    ...
    for (const code of attempts) {
        const existing = await Partner_1.Partner.findOne({ where: { referral_code: code } });
        if (!existing) {
            return code;
        }
    }
 ...
 */
function calculateTier(monthlyConversions) {
    if (monthlyConversions >= 100)
        return 'platinum';
    if (monthlyConversions >= 51)
        return 'gold';
    if (monthlyConversions >= 11)
        return 'silver';
    return 'bronze';
}
/**
