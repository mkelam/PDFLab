...
exports.getAttributionStats = exports.createPromoCode = exports.getAllPartners = exports.createPartner = exports.getPartnerReferrals = exports.getPartnerDashboard = exports.partnerLogin = void 0;
...
const partnerLogin = async (req, res) => {
    try {
        ...
        const partner = await models_1.Partner.findOne({ where: { slug } });
        if (!partner) {
            res.status(401).json({
        ...
        // Verify password
        const isValidPassword = await bcryptjs_1.default.compare(password, partner.password_hash);
        if (!isValidPassword) {
            res.status(401).json({
                error: 'Invalid credentials',
                message: 'Invalid slug or password'
        ...
        if (partner.status !== Partner_1.PartnerStatus.ACTIVE) {
            res.status(403).json({
...
const getPartnerDashboard = async (req, res) => {
    try {
        ...
        // Find partner by slug
        const partner = await models_1.Partner.findOne({
            where: { slug },
            include: [
                {
                    model: models_1.PromoCode,
                    as: 'promo_codes'
                }
            ]
        });
        ...
        const attributions = await models_1.UserAttribution.findAll({
            where: { partner_id: partner.id }
        });
        // Calculate current month stats
        const currentMonthStart = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
        const currentMonthAttributions = attributions.filter((attr) => attr.created_at >= currentMonthStart);
        ...
        const currentMonthConversions = currentMonthAttributions.filter((attr) => attr.converted_to_paid).length;
        // Calculate commission
        const totalCommissionEarned = attributions.reduce((sum, attr) => sum + parseFloat(attr.commission_due.toString()), 0);
        ...
        const recentReferrals = await models_1.UserAttribution.findAll({
            where: { partner_id: partner.id },
            include: [
                {
                    model: models_1.User,
        ...
        res.status(200).json({
            ...
            stats: {
                ...
                current_month: {
                    ...
                    conversion_rate: currentMonthSignups > 0
...
const getPartnerReferrals = async (req, res) => {
    try {
        ...
        const { rows: referrals, count: total } = await models_1.UserAttribution.findAndCountAll({
            where: whereClause,
            include: [
                {
                    model: models_1.User,
...
const createPartner = async (req, res) => {
    try {
        const { name, email, slug, platform, follower_count, website, commission_tier, free_licenses_allocated } = req.body;
        // Validation
        if (!name || !email || !slug) {
            res.status(400).json({
        ...
        const existingPartner = await models_1.Partner.findOne({
            where: {
                [sequelize_1.Op.or]: [{ slug }, { email }]
            }
        });
        if (existingPartner) {
            res.status(400).json({
        ...
        await models_1.PromoCode.create({
            partner_id: partner.id,
            code: defaultPromoCode,
            discount_type: PromoCode_1.DiscountType.PERCENTAGE,
            discount_value: 10.0,
            max_uses: 100
        });
        res.status(201).json({
...
const getAllPartners = async (req, res) => {
    try {
        const partners = await models_1.Partner.findAll({
            order: [['created_at', 'DESC']],
            include: [
                {
                    model: models_1.PromoCode,
...
const createPromoCode = async (req, res) => {
    try {
        ...
        const partner = await models_1.Partner.findByPk(id);
        if (!partner) {
            res.status(404).json({ error: 'Partner not found' });
            return;
        }
        // Check if code already exists
        const existingCode = await models_1.PromoCode.findOne({
        ...
        // Create promo code
        const promoCode = await models_1.PromoCode.create({
            partner_id: id,
 ...
 */
const getAttributionStats = async (req, res) => {
    try {
        const totalAttributions = await models_1.UserAttribution.count();
        const partnerAttributions = await models_1.UserAttribution.count({
            where: { partner_id: { [sequelize_1.Op.ne]: null } }
        });
        const organicSignups = totalAttributions - partnerAttributions;
        const convertedUsers = await models_1.UserAttribution.count({
            where: { converted_to_paid: true }
        });
        // Calculate total revenue and commission
        const allAttributions = await models_1.UserAttribution.findAll({
            where: { converted_to_paid: true }
        });
...
exports.getAttributionStats = getAttributionStats;
