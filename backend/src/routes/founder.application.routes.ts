...
import { authenticate as authenticateToken, requireAdmin } from '../middleware/auth.middleware'
...
router.post('/apply', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await query(
      'SELECT founder_application_status FROM users WHERE id = $1',
      [userId]
    )
 ...
 */
router.get('/application-stats', requireAdmin, async (req: Request, res: Response) => {
  try {
    const result = await query(`
      SELECT
 ...
 */
router.post('/approve/:userId', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    // Get user info
    const userResult = await query(
      `SELECT email, name, founder_application_status
       FROM users
       WHERE id = $1`,
      [userId]
    )
 ...
 */
router.post('/reject/:userId', requireAdmin, async (req: Request, res: Response) => {
  try {
    const { userId } = req.params
